      ...
      switchMap(action =>
        this.userService.getUsers().pipe(
          map(users => {
            ...
            if (matchedUser) {
              return LoadUserProfileSuccess({ user: matchedUser });
            } else {
              return LoadUserProfileFailure({ error: 'Invalid username or password' });
            }
          }),
          
          // 5. Catch HTTP API level errors (e.g., 500, 404 network failure)
          catchError(error =>
            of(LoadUserProfileFailure({ error: error.message || 'Server connection failed' }))
          )
        )
      )
    )
